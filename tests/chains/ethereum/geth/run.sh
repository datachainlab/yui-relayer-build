...
/usr/local/bin/geth --password /root/geth.password \
  ...
  --authrpc.vhosts "*" --authrpc.addr "0.0.0.0" --http --http.addr "0.0.0.0" --http.api web3,eth,net,personal,miner,txpool,debug --http.corsdomain '*' \
  --ws --ws.api eth,net,web3,personal,txpool --ws.addr "0.0.0.0" --ws.port "8546" --ws.origins '*' \
  --datadir /root/.ethereum --networkid "9999" --nodiscover \
