...
func NewSendPacketTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "send [src-port] [src-channel] [message]",
		Short: "Send an arbitrary message through IBC",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgSendPacket(
				srcPort, srcChannel, sender, message, timeoutHeight, timeoutTimestamp,
			)
